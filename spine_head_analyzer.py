...
class NEUROPIL_OT_output(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        dend_outFile.write('# sy pre_post spine_vol head_vol neck_vol spine_area head_area neck_area neck_cross_section_abt neck_cross_section_lbt psd_az_area neck_diameter neck_diameter_lbt neck_length neck_length_lbt psd_az_loc_x psd_az_loc_y psd_az_loc_z neck_top_loc_x neck_top_loc_y neck_top_loc_z neck_base_loc_x neck_base_loc_y neck_base_loc_z mito h_hooked h_concave h_flatface h_bulbous h_skinny n_short n_long n_stubby n_thin n_tapered n_branched n_twotiered glia_spicule glia_ensheathed glia_adjacent glia_distant exclude\n')
        ...
        axon_outFile = open('spine_head_analysis_output.axons.txt', 'wt')
...
class SpineHeadAnalyzerPSDProperty(bpy.types.PropertyGroup):
    ...
    diameter_neck = FloatProperty(name="Diameter of Neck based on area_neck_cross_section_abt",default=0.0)
    ...
    char_head_hooked = BoolProperty(name="Hooked Head",default=False)
    char_head_concave = BoolProperty(name="Concave Head",default=False)
    char_head_flatface = BoolProperty(name="Flat Face Head",default=False)
    char_head_bulbous = BoolProperty(name="Bulbous Head",default=False)
    char_head_skinny = BoolProperty(name="Skinny Head",default=False)
    char_neck_short = BoolProperty(name="Short Neck",default=False)
    char_neck_long = BoolProperty(name="Long Neck",default=False)
    char_neck_stubby = BoolProperty(name="Stubby Neck",default=False)
    char_neck_thin = BoolProperty(name="Thin Neck",default=False)
    char_neck_tapered = BoolProperty(name="Tapered Neck",default=False)
    char_neck_branched = BoolProperty(name="Branched Neck",default=False)
    char_neck_twotiered = BoolProperty(name="Twotiered Neck",default=False)
    exclude = BoolProperty(name="Exclude this spine?", default=False)
    glia_spicule = BoolProperty(name="Glia Ensheathed", default=False)
    glia_ensheathed = BoolProperty(name="Glia Ensheathed", default=False)
    glia_adjacent = BoolProperty(name="Glia Adjacent", default=False)
    glia_distant = BoolProperty(name="Glia Distant", default=False)
    ...
    def init_psd(self,context,name):
        ...
        self.char_head_hooked = False
        self.char_head_concave = False
        self.char_head_flatface = False
        self.char_head_bulbous = False 
        self.char_head_skinny = False
        self.char_neck_short = False
        self.char_neck_long = False
        self.char_neck_stubby = False
        self.char_neck_thin = False
        self.char_neck_tapered = False
        self.char_neck_branched = False
        self.char_neck_twotiered = False
        self.exclude = False
        self.glia_spicule = False
        self.glia_ensheathed = False
        self.glia_adjacent = False
        self.glia_distant = False
...
#        print("    Area of %s with %d faces is %g" % (region_name,len(reg_faces),area))
    ...
    def compute_areas(self,context):
        ...
        region_name = self.name.replace('sy','axb')
        if reg_list.get(region_name) != None:
            self.area_head = self.compute_region_area(context,region_name)
    ...
    def compute_neck_boundary_area(self, context, reg_name):
      ...
      dup_obj.name = reg_name + '_neckbound'
      ...
      v_array = []
      for v in verts:
        v_array.append([v.co.x, v.co.y, v.co.z])
      
      # Do PCA on the vertices
      v_array = np.array(v_array)
      mpt = v_array.mean(axis=0)
      covm = np.cov(v_array,rowvar=0)
...
#              self.compute_psd_az_location(context)
        ...
        file.write(' %g' % (self.area_spine))
        file.write(' %g' % (self.area_head))
        file.write(' %g' % (self.area_neck))
        file.write(' %g' % (self.area_neck_cross_section_abt))
        file.write(' %g' % (self.area_neck_cross_section_lbt))
        file.write(' %g' % (self.area_psd_az))
        file.write(' %g' % (self.diameter_neck))
        file.write(' %g' % (self.diameter_neck_lbt))
        file.write(' %g' % (self.length_neck))
        file.write(' %g' % (self.length_neck_lbt))
        file.write(' %g' % (self.psd_az_location[0]))
        file.write(' %g' % (self.psd_az_location[1]))
        file.write(' %g' % (self.psd_az_location[2]))
        file.write(' %g' % (self.neck_top_location[0]))
        file.write(' %g' % (self.neck_top_location[1]))
        file.write(' %g' % (self.neck_top_location[2]))
        file.write(' %g' % (self.neck_base_location[0]))
        file.write(' %g' % (self.neck_base_location[1]))
        file.write(' %g' % (self.neck_base_location[2]))
        file.write(' %s' % (self.char_mito))
        file.write(' %s' % (self.char_head_hooked))
        file.write(' %s' % (self.char_head_concave))
        file.write(' %s' % (self.char_head_flatface))
        file.write(' %s' % (self.char_head_bulbous))
        file.write(' %s' % (self.char_head_skinny))
        file.write(' %s' % (self.char_neck_short))
        file.write(' %s' % (self.char_neck_long))
        file.write(' %s' % (self.char_neck_stubby))
        file.write(' %s' % (self.char_neck_thin))
        file.write(' %s' % (self.char_neck_tapered))
        file.write(' %s' % (self.char_neck_branched))
        file.write(' %s' % (self.char_neck_twotiered))
        file.write(' %s' % (self.glia_spicule))
        file.write(' %s' % (self.glia_ensheathed))
        file.write(' %s' % (self.glia_adjacent))
        file.write(' %s' % (self.glia_distant))
        file.write(' %s' % (self.exclude))
        file.write('\n')



    def compute_volume(self,context,mode,n_components,make_shell_opt):
...
#                            pass
                ...
                if (psd.volume < 0.0):
                    print('  ***** Negative head volume for PSD %s %g' % (psd_region_name, psd.volume))
                ...
                if (psd.volume_neck > 0.0):
                    ...
                    report_file.write('  Updated diameter and length of neck %s  diameter: %g   length: %g\n' % (neck_region_name, psd.diameter_neck, psd.length_neck))
...
#FIXME: add "*sy*" filter:
            ...
            if psd != None:
                if psd.char_postsynaptic:
                    ...
                    row = layout.row()
                    row.prop(psd,"glia_spicule",text="Spicule Ensheathed")
                    row = layout.row()
                    row.prop(psd,"glia_ensheathed",text="Glia Ensheathed")
                    row = layout.row()
                    row.prop(psd,"glia_adjacent",text="Glia Adjacent")
                    row = layout.row()
                    row.prop(psd,"glia_distant",text="Glia Distant")
                    ...
                    if (psd.volume != 0.0):
                        row = layout.row()
                        row.label(text="Spine Head: %s Volume: %.4g" % (psd.head_name, psd.volume))
                    row = layout.row()
                    row.prop(psd,"char_head_hooked",text="Hooked")
                    row = layout.row()
                    row.prop(psd,"char_head_concave",text="Concave")
                    row = layout.row()
                    row.prop(psd,"char_head_flatface",text="Flat Face")
                    row = layout.row()
                    row.prop(psd,"char_head_bulbous",text="Bulbous")
                    row = layout.row()
                    row.prop(psd,"char_head_skinny",text="Skinny")
#                    row = layout.row()
#                    row.prop(psd,"ensheathment",text="Glial Ensheathment")
                    ...
                    if (psd.volume_spine != 0.0):                    
                        row = layout.row()
                        row.label(text="Spine: %s Volume: %.4g" % (psd.spine_name, psd.volume_spine))
                    if (psd.volume_neck != 0.0):                    
                        row = layout.row()
                        row.label(text="Spine Neck: %s Volume: %.4g" % (psd.neck_name, psd.volume_neck))
                    row = layout.row()
                    row.prop(psd,"char_neck_short",text="Short")
                    row = layout.row()
                    row.prop(psd,"char_neck_long",text="Long")
                    row = layout.row()
                    row.prop(psd,"char_neck_stubby",text="Stubby")
                    row = layout.row()
                    row.prop(psd,"char_neck_thin",text="Thin")
                    row = layout.row()
                    row.prop(psd,"char_neck_tapered",text="Tapered")
                    row = layout.row()
                    row.prop(psd,"char_neck_branched",text="Branched")
                    row = layout.row()
                    row.prop(psd,"char_neck_twotiered",text="Two-tiered")
                else:
                    row = layout.row()
